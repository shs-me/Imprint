...
def set_ax_pct(ax: Axes, base_tick: float) -> None:
    ax_pct: Axes = ax.twinx()  # pyright: ignore[reportUnknownMemberType])
    ymin, ymax = ax.get_ylim()
    pct_min: float = ((ymin - base_tick) / base_tick) * 100.0
    pct_max: float = ((ymax - base_tick) / base_tick) * 100.0
    ax_pct.set_ylim(pct_min, pct_max)
    ax_pct.tick_params(  # pyright: ignore[reportUnknownMemberType])
        axis="y",
        labelcolor="#757575",
        labelsize=8,
    )
...
def set_base_title(ax: Axes, text: str) -> None:
