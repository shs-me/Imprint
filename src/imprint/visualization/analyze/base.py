...
class Stats:
    ...
    payoff_ratio: float = field(init=False)
    calmar_ratio: float = field(init=False)
    max_win_streak: int = field(init=False)
    max_loss_streak: int = field(init=False)
    sqn: float = field(init=False)
    kelly_pct: float = field(init=False)
    turnover: float = field(init=False)
    long_count: int = field(init=False)
    long_wr: float = field(init=False)
    long_pnl: float = field(init=False)
    short_count: int = field(init=False)
    short_wr: float = field(init=False)
    short_pnl: float = field(init=False)
    ...
    def __post_init__(self) -> None:
        ...
        self.max_win_streak, self.max_loss_streak = calculate_streaks(
            self.trades_close
        )
        self.sqn = calculate_sqn(self.all_pnls)
        ...
        (
            ...
            self.short_wr,
            self.short_pnl,
        ) = calculate_directional_stats(self.trades_close)
