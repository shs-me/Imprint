...
class Converter:
    def __init__(
        ...
        cfgStrategy: cfg.RiskManagement,
        price_prec: int,
        qty_prec: int,
    ) -> None:
        ...
        self.cfgST: cfg.RiskManagement = cfgStrategy
        self._entryQty: int = self.cfgST.entry_qty.fixed
        self._tpDev: int = self.cfgST.tp_dev.fixed
        self._slDev: int = self.cfgST.sl_dev.fixed
        self._max_lock_balance: int = self.cfgST.max_lock_balance.fixed
        self._max_loss_balance: int = self.cfgST.max_loss_balance.fixed
        ...
        self.takerNcommission: int = self.cfgAC.taker_commission.fixed
        self.makerNcommission: int = self.cfgAC.maker_commission.fixed
    ...
    def tp_sl_param(
        ...
        order_param |= (
            (c.OF_LIMIT if is_tp else c.OF_MARKET_TRIGGER) | c.OF_NEW | c.OF_OCO
        )
    ...
    def is_averaging(self, order_param: int) -> bool:
        ...
        if is_buy and is_long:
            return bool(self._longNqty[0] or self.have_pending_orders)
        elif not is_buy and not is_long:
            return bool(self._shortNqty[0] or self.have_pending_orders)
        else:
            return False
