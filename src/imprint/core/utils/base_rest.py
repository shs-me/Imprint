...
from imprint.core.settings import LoggerProtocol, SetLogMethodSignature
...
class BaseREST(ABC):
    ...
    def format_bytes(self, size: float) -> str:
        i: int = 0
        while size >= 1024 and i < (len(self._units) - 1):
            size, i = size / 1024, i + 1

        return f"{size:.2f} {self._units[i]}"
    ...
    def _prepare_payload(
        ...
        if json_body is not None:
            if isinstance(json_body, (str, bytes)):
                ...
                encoded_bytes = self._encoder.encode(json_body)
    ...
    def _decode_response(
        ...
        # Return raw bytes if requested explicitly
